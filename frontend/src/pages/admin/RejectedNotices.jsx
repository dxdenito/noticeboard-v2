...
export default function RejectedNotices() {
  ...
  async function load() {
    setLoading(true);
    try {
      const data = await api.get(`/notices/rejected?limit=${PAGE_SIZE}&offset=${page * PAGE_SIZE}`);
      setNotices(data);
    } catch (err) {
  ...
  return (
    <div>
      <h1 className="text-2xl font-extrabold text-gray-900 mb-4">Rejected Notices</h1>
      {notices.length === 0 && (
        <p className="text-sm text-gray-400">Nothing rejected.</p>
      )}
